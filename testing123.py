...
def hello():
    if request.method == "POST":
        ...
        return render_template("index.html",
                               self_consumption_50=c.get_yearly_revenue(
                                   area, .5),
                               self_consumption_30=c.get_yearly_revenue(
                                   area, .3),
                               self_consumption_15=c.get_yearly_revenue(
                                   area, .15),
                               panel_cost=c.get_panel_costs_by_area(area),
                               inverter_cost=c.get_inverter_costs(area),
                               installation_cost=c.get_initial_investment_costs(
                                   area),
